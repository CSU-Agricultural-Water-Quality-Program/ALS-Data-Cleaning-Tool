...
create_violinplot <- function(df, selected_location) {
  ...
  filtered_df <- filter_dataframe(df, selected_location)
  ...
  dropdown_buttons <- lapply(unique(y_axis_var_names), function(analyte_name) {
    y_data <- filtered_df$result[filtered_df$analyte == analyte_name]
  ...
  location_violin_plot <- plot_ly(data = filtered_df,
                                  x = ~event.type,
                                  y = ~result,
                                  color = ~event.type,
                                  colors = event_colors,
...
create_scatterplot <- function(df, selected_location) {
  ...
  location_scatter_plot <- plot_ly(data = filtered_df, x = ~collected, y = ~result,
