...
create_facet_bar_plot <- function(df, output_dir, file_name, x_axis_var) {
  tryCatch({
    # Ensure the output directory exists
    if (!dir.exists(output_dir)) {
      dir.create(output_dir, recursive = TRUE)
    ...
    # Generate faceted bar plot with custom facet labels and matching color palette
    facet_bar_plot <- ggplot(summary_df, aes(x = !!sym(x_axis_var), y = mean, fill = !!sym(x_axis_var))) +
      ...
      geom_jitter(data = df, aes(x = !!sym(x_axis_var), y = result), 
                  width = 0.2, alpha = 0.6, size = 1, show.legend = FALSE) +  # Jitter to avoid overlap
      # TODO: play with adding sampler method in facet_wrap
...
# Call the updated function with the full dataset (filtered_df)
facet_bar <- create_facet_bar_plot(filtered_df,
                                   output_dir = "./site_selector_temporary_results",
