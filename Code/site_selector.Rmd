...
See comments below for event.count
```{r, eval=FALSE}
## Original code
create_and_save_boxwhisker_plot <- function(df, output_dir, file_name, plot.title, x_axis_var) {
  tryCatch({
    ...
    bw_plot <- ggplot(df, aes_string(x = x_axis_var, y = "result", fill = x_axis_var)) +
      ...
      ### Change event.count to a date without time 
...
####### Abby's Edits – Fixed Version #######

library(ggplot2)
library(dplyr)
library(tools)  # ensures the namespace is available

# Helper function to inspect dropped rows (optional)
find_dropped_rows <- function(df, x_axis_var) {
  df %>%
    dplyr::mutate(
      collected_date = as.Date(collected),
      result_num = suppressWarnings(as.numeric(result)),
      x_val = .data[[x_axis_var]],
      bad_result = !is.finite(result_num),
      bad_x = is.na(x_val) | (is.character(x_val) & x_val == "")
    ) %>%
    dplyr::filter(bad_result | bad_x) %>%
    dplyr::select(analyte.abbr, collected, collected_date, result, result_num,
                  !!x_axis_var, bad_result, bad_x)
}
...
create_and_save_boxwhisker_plot <- function(df, output_dir, file_name, plot.title, x_axis_var) {
  tryCatch({
    ...
    df <- df %>%
      ...
      dplyr::filter(is.finite(result), !is.na(.data[[x_axis_var]]))

    # Guard: stop if empty
    if (nrow(df) == 0) {
      stop("No finite 'result' values (and/or missing x) after filtering; nothing to plot.")
    }

    # Order facet columns chronologically
    df <- df %>%
      dplyr::arrange(collected_date) %>%
      dplyr::mutate(collected_date = factor(collected_date, levels = unique(collected_date)))

    # Create output file paths (explicit namespace for safety)
    file_path_portrait  <- file.path(output_dir, file_name)
    file_path_landscape <- file.path(output_dir, paste0(tools::file_path_sans_ext(file_name), "_landscape.jpg"))

    # --- Plot ---
    bw_plot <- ggplot(df, aes(x = .data[[x_axis_var]], y = result, fill = .data[[x_axis_var]])) +
      geom_boxplot(position = position_dodge(0), na.rm = TRUE) +
      geom_jitter(width = 0.2, size = 1.5, alpha = 0.6, na.rm = TRUE) +
      facet_grid(analyte.abbr ~ collected_date, scales = "free_y", space = "free_x") +
      theme_minimal() +
      theme(
        axis.text.x = element_text(angle = 45, hjust = 1, size = 14),
        axis.text.y = element_text(size = 14),
        axis.title  = element_text(size = 16),
        strip.text.x = element_text(size = 10),  # date facets (top)
        strip.text.y = element_text(size = 14),  # analyte facets (side)
...
# Example calls
bw_N <- create_and_save_boxwhisker_plot(
  N_df,
  output_dir = "./site_selector_temporary_results",
  file_name = "bw_N.jpg",
  plot.title = "Forms of Nitrogen",
...
bw_P <- create_and_save_boxwhisker_plot(
  P_df,
  output_dir = "./site_selector_temporary_results",
  file_name = "bw_P.jpg",
  plot.title = "Forms of Phosphorous",
...
bw_Misc <- create_and_save_boxwhisker_plot(
  Misc_df,
  output_dir = "./site_selector_temporary_results",
  file_name = "bw_Misc.jpg",
  plot.title = "Water Quality Analytes",
