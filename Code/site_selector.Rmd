...
create_scatterplot_matrix <- function(df, location_name) {
  ...
  tryCatch({
    ...
    filtered_data <- df %>%
      mutate(sample.id = sub("-[1-5]$", "", sample.id),
             sample.id = sub("-[1-5]-D$", "-D", sample.id)) %>%
...
create_and_save_boxwhisker_plot <- function(df, output_dir = "./site_selector_temporary_results", file_name = "bw_plot.png") {
  tryCatch({
    ...
    bw_plot <- ggplot(df, aes(x = factor(event.type), y = result, fill = factor(event.type))) +
      geom_boxplot(outlier.shape = NA, position = position_dodge(0)) + # Hide outliers for clarity
...
if (!is.null(bw_plot)) {
  ...
  print("The plot could not be generated due to an error.")
}
