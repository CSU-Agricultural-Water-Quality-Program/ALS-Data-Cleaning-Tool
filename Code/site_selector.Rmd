  ...
  tryCatch({
    ...
    df <- df %>%
      mutate(
        ...
        event_facet  = as.character(event_facet),
        treatment.name = as.character(treatment.name)
      ) %>%
      filter(is.finite(result)) %>%
      filter(!is.na(analyte.abbr), !is.na(event_facet), !is.na(treatment.name))
    ...
    # Build one ggplotly per analyte (TRUE boxplots + jitter)
    plot_tags <- lapply(seq_along(analytes), function(i) {
      ...
      g <- ggplot(
        ...
        geom_boxplot(position = position_dodge(0), na.rm = TRUE, outlier.shape = NA) +
        ...
        labs(
          ...
          subtitle = paste("Analyte:", a),
          x = "Event Type",
          y = "Result",
          fill = "Event Type"
        )

      p <- ggplotly(g, tooltip = c("y")) %>%
        layout(yaxis = list(autorange = TRUE))  # ensure initial autorange
    ...
    # Dropdown (plain HTML select; works in Rmd HTML)
    selector <- tags$label(
      `for` = "analyte_select",
      "Select analyte: ",
      tags$select(
    ...
    # JS: show the chosen plot, hide others, and force Plotly to autoscale
    js <- sprintf(
      "
      function showPlot(idx){
        var n = %d;
        for (var i=1; i<=n; i++){
          var el = document.getElementById('plot_' + i);
          if (!el) continue;
          el.style.display = (i === idx) ? '' : 'none';
        }
        // Force plotly to re-autorange after showing
        setTimeout(function(){
          var el = document.getElementById('plot_' + idx);
          if (!el) return;
          var gd = el.querySelector('.plotly-graph-div');
          if (gd && window.Plotly){
