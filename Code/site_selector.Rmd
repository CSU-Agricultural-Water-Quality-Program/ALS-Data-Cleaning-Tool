...
create_and_save_boxwhisker_plot <- function(df, output_dir, file_name) {
  tryCatch({
    ...
    bw_plot <- ggplot(df, aes(x = factor(event.type), y = result, fill = factor(event.type))) +
      ...
      labs(x = "Event Type", y = "Result", fill = "Event Type") +
      scale_fill_brewer(palette = "Set3") # Colorblind-friendly palette
...
# Example usage:
# Assuming 'filtered_df' is your data frame after filtering
# bw_plot <- create_and_save_boxwhisker_plot(filtered_df,
#                                            output_dir = "./site_selector_temporary_results",
#                                            file_name = "bw_plot1.png")
# bw_plot
analytes_plot1 <- c("Nitrogen, Nitrate (As N)", 
                    "Nitrogen, Nitrite  (As N)", # note the extra space before "("
...
plot1_df <- filtered_df %>% filter(analyte %in% analytes_plot1)
plot2_df <- filtered_df %>% filter(analyte %in% analytes_plot2)
plot3_df <- filtered_df %>% filter(analyte %in% analytes_plot3)

bw_plot1 <- create_and_save_boxwhisker_plot(plot1_df,
                                            output_dir = "./site_selector_temporary_results",
                                            file_name = "bw_plot1.png")
bw_plot2 <- create_and_save_boxwhisker_plot(plot2_df,
                                            output_dir = "./site_selector_temporary_results",
                                            file_name = "bw_plot2.png")
bw_plot3 <- create_and_save_boxwhisker_plot(plot3_df,
                                            output_dir = "./site_selector_temporary_results",
                                            file_name = "bw_plot3.png")
...
```{r}
datatable(filtered_df, colnames = c('Sample ID', 'Lab ID', 'Method', 'CAS Number', 'Analyte', 'Result', 'Units', 'Dilution', "Results Reported To", 'MDL', 'RL', 'Report Basis', 'Moisture (%)', 'Solid (%)', 'Non detect', 'Duplicate', 'Location', 'Treatment', 'Method Name', 'Event Type', 'Lab ID', 'Matrix', 'Collection Date', 'Recieved Date', 'Hold Time', 'Flag'))
```
