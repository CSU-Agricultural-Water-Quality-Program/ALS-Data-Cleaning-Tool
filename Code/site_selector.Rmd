...
# Number of events captured at this site:
```{r}
# Combine event.count values into a single row for each treatment.name
unique_treatments <- filtered_df %>%
  select(treatment.name, event.count) %>%
  distinct() %>%
  group_by(treatment.name) %>%
  summarize(event.count = paste(event.count, collapse = ", ")) %>%
  arrange(treatment.name)

# Print the table using datatable
datatable(unique_treatments, 
              caption = "Unique Treatments and Corresponding Event Counts")
  ...
  tryCatch({
    ...
    location_violin_plot <- plot_ly(data = df,
      ...
      layout(
        ...
        xaxis = list(title = "Sample Location"),  # Update x-axis title
        yaxis = list(title = "Result (mg/L, ug/L, dS/m, or pH)"),
        showlegend = TRUE,
        updatemenus = list(
          list(
...
create_scatterplot <- function(df, selected_location) {
  ...
  tryCatch({
    ...
    create_buttons <- function(y_axis_var_name) {
      analyte_data <- df[df$analyte.abbr == y_axis_var_name, ]
    ...
    first_analyte_data <- df[df$analyte.abbr == first_analyte, ]
...
create_and_save_bar_plot <- function(df, output_dir, file_name, plot.title, x_axis_var) {
  tryCatch({
    ...
    # Generate the bar plot with error bars and dynamic x-axis
    bar_plot <- ggplot(summary_df, aes_string(x = x_axis_var, y = "mean", fill = x_axis_var)) +
