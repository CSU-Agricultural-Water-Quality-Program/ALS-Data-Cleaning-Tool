...
create_violin_plot <- function(df, location_name) {
  ...
  create_buttons <- function(df, y_axis_var_name, location_name) {
    list(
      method = 'restyle',
      args = list('y', list(df[df$analyte == y_axis_var_name & df$location.name == location_name, "result"])),
  ...
  location_violin_plot <- plot_ly(df, x = ~event.type,
    ...
    layout(
      ...
      updatemenus = list(
        list(
          buttons = lapply(y_axis_var_names, create_buttons, df = df, location_name = location_name)
        )
      ))
