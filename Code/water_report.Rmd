...
output_file <- paste0("Report_", format(Sys.Date(), "%Y-%m-%d"), ".html")
output_path <- "./Report"  # Replace with the desired folder path

# Render the RMD file and save it in the existing folder
render("water_report.Rmd", output_format = "html_document", output_file = file.path(output_path, output_file))
```
```
  ...
  for (location in unique_locations) {
    for (analyte in unique_analytes) {
      ...
      filtered_data <- data %>%
        ...
        ungroup()
       units <- unique(filtered_data$units)
      # Create the scatter plot
      scatter_plot <- ggplot(filtered_data, aes(x = collected, y = result, color = event.type)) +
        geom_point() +
        labs(x = "Date", y = paste("Result", units), title = paste( analyte, "Results for", location))
