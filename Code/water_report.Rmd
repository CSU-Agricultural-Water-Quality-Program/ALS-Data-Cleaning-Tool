...
make_scatter_plots <- function(data) {
  ...
  analyte_units <- unique(data[, c("analyte", "units")])
  ...
  for (location in unique_locations) {
    for (analyte in unique_analytes) {
      ...
      filtered_data <- data %>%
        ...
        ungroup()
      #make sure the graphs do not show just time 
      filtered_data$collected <- format(filtered_data$collected, "%Y-%m-%d")
      
       
     analyte_units_current <- analyte_units$units[analyte_units$analyte == analyte]
     
   if (any(is.na(analyte_units_current)) || any(analyte_units_current == "N/A")) {
        analyte_units_current[is.na(analyte_units_current) | analyte_units_current == "N/A"] <- ""
      }
      # Create the scatter plot
      scatter_plot <- ggplot(filtered_data, aes(x = collected, y = result, color = event.type)) +
        geom_point() +
        labs(x = "Date", y = paste("Result", analyte_units_current), title = paste( analyte, "Results for", location)) +
        theme_bw()
